...
// onMounted는 이 화면이 브라우저에 장착(Mount)되는 순간을 감지해서 내부 코드를 즉시 실행합니다.
onMounted(() => {
    store.fetchovies();
});
</script>
...
<template>
    <main class="page">
        ...
        <div v-else-if="store.errorMessage" class="status-message arror">
            🚨 {{ store.errorMessage }}
        </div>
...
.header-section { margin-bottom: 30px; text-align: conter; color: #2c3e50; }
...
.movie-card { border-radius: 12px; overflow: hidden; background: white; text-align: left; box-shadow: 0 4px 15px 
...
.poster-placeholder { width: 100%; height: 300px; background-color: #ddd; display: flex; align-items: center; 
justify-content: center; color: #7f8c8d; font-weight: bold; }
.card-content { padding: 20px; display: flex; flex-direction: column; flex-grow: 1; }
.title { font-size: 18px; color: #333; margin: 0 0 6px 0; white-space: nowrap; overflow: hidden; text-overflow: 
...
.fav-btn { width: 100%; padding: 12px; cursor: pointer; border: none; background: #ecf0f1; color: #333; 
